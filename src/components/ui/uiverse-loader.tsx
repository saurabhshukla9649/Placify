...
export const UiverseLoader: React.FC<UiverseLoaderProps> = ({ 
  ...
  const letters = text.split("").map((char, index) => (
    <span key={index} className="loader-letter font-headline font-bold text-lg text-primary tracking-widest px-0.5">
      {char === " " ? "\u00A0" : char}
    </span>
  ));
  ...
  return (
    <div className={`relative flex items-center justify-center w-full h-full min-h-[300px] ${className}`}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        @keyframes loader-rotate {
          0% {
            ...
            box-shadow:
              0 10px 20px 0 rgba(255, 255, 255, 0.4) inset,
      ...
      <div className="loader-wrapper">
        ...
        <div className="flex z-10">{letters}</div>
      </div>
    </div>
  );
};
